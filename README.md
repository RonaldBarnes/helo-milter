...
3. Edit `helo-milter.service`:
   ...
   * Uncomment `ExecStart=python3 helo-milter.py`
...
with 0 (Success) unless signal's numeric value is 10 (SIGUSR1) which is then the exit code.
...
The unit file's `RestartForceExitStatus=10` will force a restart on exit code 1 (`SIGUSR1`).
